...
from ap2.connections.NTP import NTP
        ...
        for k in self.sdp:
            if 'v=' in k:
                ...
                if self.audio_encoding == 'AppleLossless':
                    ...
                    # self.audio_format_ch = int(self.audio_format_ch)
                    # self.audio_format_sr = int(self.audio_format_sr)
                    # self.audio_format_bd = [int(i) for i in self.audio_format_bd.split() if i.isdigit()]
...
class AP2Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_RECORD(self):
        ...
        # Interesting headers:
        # Range: npt=0-
        # RTP-Info: seq=12345;rtptime=872345786
        if self.headers["Content-Type"] == HTTP_CT_BPLIST:
            content_len = int(self.headers["Content-Length"])
            if content_len > 0:
                body = self.rfile.read(content_len)
