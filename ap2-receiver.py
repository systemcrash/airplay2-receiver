    ...
    def do_SETUP(self):
        ...
        if self.headers["Content-Type"] == HTTP_CT_BPLIST:
            ...
            if content_len > 0:
                ...
                if self.server.session.getTimingProtocol() == 'PTP' and not self.server.timing_proc:
        ...
        if self.headers["Content-Type"] == HTTP_CT_BPLIST:
            ...
            try:
                if content_len > 0:
                    ...
                    self.server.session.setRateAnchorTimePTP(plist)
                    timelineinfo = None
                    ...
                    """
                    try:
                        if self.server.session.getTimingProtocol() == 'PTP' and self.server.timing_proc:
                            ...
                            """
                            timelineinfo = self.server.session.getTimelineInfo(master_Nanos, master_Correction)
