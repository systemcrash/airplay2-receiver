...
class FPAES():
    def __init__(self, fpaeskey=None, rsaaeskey=None, aesiv=None):
        if rsaaeskey:
            ...
            #     randfunc=Random.get_random_bytes,
            # )
            # self.aeskey = cipher.decrypt(self.aeskey)
            """
...
0: load AIRPORT_PRIVATE_KEY (OpenSSL::RSA->new_private_key)
1: base64 decode aesiv
2: base64 decode rsaaeskey
3: decrypt rsaaeskey using 0 (use_pkcs1_oaep_padding)
4: Decrypt packets:
4a: AES_set_decrypt_key(aeskey, 128, &aes);
4b: memcpy(iv, aesiv, sizeof(iv)); // iv is char[16]
    AES_cbc_encrypt((unsigned char*)buf, packet, aeslen, &aes, iv, AES_DECRYPT);

So in python (with Pycryptodome):
